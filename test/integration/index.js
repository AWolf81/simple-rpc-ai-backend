...
async function testRPCIntegration() {
  ...
  try {
    ...
    const promptIdTestResult = await client.request('executeAIRequest', {
    ...
    try {
      const managedPromptResult = await client.request('executeAIRequest', {
        content: 'function add(a, b) { return a + b; }',
    ...
    const directPromptResult = await client.request('executeAIRequest', {
    ...
    try {
      ...
      const securityResult = await client.request('executeAIRequest', {
        content: testCode,
      ...
      const performanceResult = await client.request('executeAIRequest', {
        content: testCode,
    ...
    for (const prompt of prompts) {
      try {
        const promptResult = await client.request('executeAIRequest', {
          content: 'function add(a, b) { return a + b; }',
          promptId: prompt.id,

...
describe('ClientManagedStorageAdapter', () => {
  ...
  describe('API key operations', () => {
    it('should throw error when trying to store API key', async () => {
      await expect(
        adapter.storeApiKey('anthropic', 'sk-ant-test-key', 'user@test.com')
      ).rejects.toThrow('Client-managed storage does not support server-side key storage');
    });
    ...
    it('should throw error when trying to delete API key', async () => {
      await expect(
        adapter.deleteApiKey('anthropic', 'user@test.com')
      ).rejects.toThrow('Client-managed storage does not support server-side key deletion');
    });
    ...
    it('should throw error when trying to rotate API key', async () => {
      await expect(
        adapter.rotateApiKey('anthropic', 'new-api-key', 'user@test.com')
      ).rejects.toThrow('Client-managed storage does not support server-side key rotation');
    });

...
// Types that aren't exported from compiled dist
...
describe('SecurityLogger', () => {
  ...
  beforeEach(() => {
    ...
    mockNext = vi.fn().mockImplementation(() => {}) as NextFunction;
    ...
    mockLogger = (winston.createLogger as any).mock.results[0].value;
  ...
  afterEach(() => {
    ...
    if (securityLogger) {
      (securityLogger as any).isLogging = false;
    }
  });
  ...
  describe('Security Event Logging', () => {
    it('should log security events with all required fields', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(
        'info', // LOW severity maps to info level
  ...
  describe('Network Filter Middleware', () => {
    let networkMiddleware: (req: AuthenticatedRequest, res: Response, next: NextFunction) => Promise<void>;
  ...
  describe('MCP Logging Middleware', () => {
    let mcpMiddleware: (req: AuthenticatedRequest, res: Response, next: NextFunction) => Promise<void>;
  ...
  describe('Malicious Pattern Detection', () => {
    it('should detect command injection patterns', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(
        'error',
        'Command injection attempt detected in MCP request',
        expect.objectContaining({
    ...
    it('should detect template injection patterns', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(
        'error',
        'Template injection attempt detected in MCP request',
        expect.objectContaining({
    ...
    it('should detect system override attempts', async () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(
        'error',
        'System override attempt detected in MCP request',
        expect.objectContaining({

...
describe('MCPRateLimiter - Throttling & Load Management', () => {
  ...
  let mockRequest: { ip: string; headers: {}; user: any; body?: any };
  let mockResponse: { status: any; json: any; set: any };
  let mockNext: () => void;

  const defaultConfig: Partial<MCPRateLimitConfig> = {
    enabled: true,
    adaptive: {
      enabled: true,
      cpuThreshold: 80,
  ...
  describe('Load Level Detection', () => {
    it('should detect normal load (level 0)', () => {
      ...
      vi.spyOn(rateLimiter as any, 'systemStats', 'get').mockReturnValue({
  ...
  describe('Graduated Throttling', () => {
    ...
    it('should not throttle under normal load', () => {
      // Mock normal load
      vi.spyOn(rateLimiter as any, 'getLoadReductionLevel').mockReturnValue(0);
    ...
    it('should apply light throttling (20% reduction)', () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Load level 1 - throttling tool \'ai-request\' from 50 to 40 requests')
      );
    });
    ...
    it('should apply moderate throttling (40% reduction)', () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Load level 2 - throttling tool \'ai-request\' from 50 to 30 requests')
      );
    });
    ...
    it('should apply heavy throttling (60% reduction)', () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('Load level 3 - throttling tool \'ai-request\' from 50 to 20 requests')
      );
    });
  });
  ...
  describe('CPU Usage Monitoring', () => {
    ...
    it('should handle first CPU measurement gracefully', () => {
      ...
      const freshLimiter = new MCPRateLimiter(freshConfig) as any;
  ...
  describe('Error Handling', () => {
    it('should handle CPU monitoring errors gracefully', () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        '❌ Rate limiting: Failed to update system stats:',
        expect.any(Error)
      );
  ...
  describe('Integration with Express Rate Limiting', () => {
    ...
    it('should prioritize authenticated users during throttling', () => {
      // Mock moderate load
      vi.spyOn(rateLimiter as any, 'getLoadReductionLevel').mockReturnValue(2);

...
**What to verify:**
- ✅ `metadata` contains frontmatter data
- ✅ `instructions` contains SKILL.md body
- ✅ `level1Tokens` and `level2Tokens` shown
- ✅ `scripts` array has 2 scripts
- ✅ `allowedPaths` is ["/workspace", "/tmp"]
...
**What to verify:**
- ✅ `level1` (metadata) ~85 tokens
- ✅ `level2` (instructions) ~200 tokens
- ✅ `level3` = 0 (resources not loaded yet)
- ✅ `breakdown` shows per-section counts
...
**What to verify:**
- ✅ `exitCode: 0`
- ✅ `stdout` contains formal greeting
...
**What to verify:**
- ✅ Returns 1 skill (hello-world)
- ✅ Has `capabilities: ["testing", "demonstration"]`
...
- ✅ `bySource.builtin: 1`
- ✅ `bySource.local: 2`
- ✅ `totalLevel1Tokens` < 300
- ✅ `totalLevel2Tokens` < 10000

## Test 11: Using simple-agent CLI (Interactive)

The simple-agent CLI can interact with the skills system:

```bash
# Install simple-agent globally (if not already)
npm link
...
> Tell me about the hello-world skill
> Execute the greet script with name "Charlie"
```

...
class AICodeHelper {
   ...
   */
  async analyzeCode(code: string, language = 'javascript') {
    try {
   ...
   */
  async generateTests(code: string, framework = 'vitest', testType = 'unit') {
    try {
   ...
   */
  async reviewPR(diff: string, focusArea?: string) {
    try {
   ...
   */
  async generateDocs(code: string, format = 'markdown', audience = 'intermediate') {
    try {
   ...
   */
  async optimizePerformance(code: string, platform = 'node.js') {
    try {
...
export class VSCodeAIHelper extends AICodeHelper {
  /**
   * Analyze the currently selected code in VS Code
   */
  async analyzeSelection(editor: any) {
   ...
   */
  async generateTestsForFile(editor: any) {

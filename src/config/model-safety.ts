...
export interface ModelSafetyConfig {
  useRegistry: boolean;           // Use live registry data
  allowPriceUpdates: boolean;     // Allow live pricing updates
  requireManualApproval: boolean; // Require manual approval for changes
  maxPriceChange: number;         // Maximum allowed price change ratio
  validationInterval: number;     // How often to check for updates (ms)
  fallbackToStatic: boolean;      // Fall back to static definitions on error
  validationMode: 'strict' | 'warn' | 'off'; // How to handle validation failures
}
 ...
 */
export function getModelSafetyConfig(): ModelSafetyConfig {
 ...
 */
export function createCustomSafetyConfig(overrides: Partial<ModelSafetyConfig>): ModelSafetyConfig {
...
export class ModelValidator {
  static validateModelData(models: any[]): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  static validatePriceChange(oldPrice: number, newPrice: number, maxChange: number): boolean {
    if (!oldPrice || !newPrice) return true; // Skip validation if no pricing data

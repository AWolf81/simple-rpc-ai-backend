...
class MCPProtocolHandler {
    ...
    buildPromptVariableDefinitions(config) {
        const variables = {};
        if (config.variables) {
        ...
        try {
            ...
            for (const [fullName, procedure] of Object.entries(allProcedures)) {
                ...
                if (meta?.mcpPrompt) {
                    ...
                    const sanitizedDescription = this.sanitizeDescription(config.description || `Prompt: ${config.name}`);
                    prompts.push({
                        name: config.name,
                        description: sanitizedDescription,
    ...
    async handleToolsCall(request, req) {
        try {
            ...
            if (!isPublic && !isAdmin && (!req?.user || !hasRequiredScopes)) {
                const requiresAdminUser = tool.requireAdminUser || Boolean(tool.scopes?.adminUsers);
    ...
    async handlePromptsGet(request, req) {
        try {
            ...
            let legacyPrompt = null;
            if (!prompt && this.extensionsConfig?.prompts?.customPrompts) {
                legacyPrompt = this.extensionsConfig.prompts.customPrompts.find((p) => p.name === name);
                if (legacyPrompt) {
                    isLegacyPrompt = true;
                }
            }
    ...
    handleRootsListChanged(request) {
    ...
    async handleResourcesTemplatesList(request, req) {
        try {
            // Check auth requirements if enabled
            if (this.authConfig.requireAuthForToolsList && !req?.user) {

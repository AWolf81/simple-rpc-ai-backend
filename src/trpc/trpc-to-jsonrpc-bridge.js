...
export class TRPCToJSONRPCBridge {
     ...
     */
    resolveProcedureFromPath(caller, path) {
        try {
     ...
     */
    extractDescription(procedureDef) {
            ...
            if (inputSchema._def?.typeName === 'ZodObject' && inputSchema._def?.shape) {
                ...
                for (const [key, fieldSchema] of Object.entries(shape)) {
                    const param = {
                        ...
                        schema: this.zodSchemaToOpenRPCSchema(fieldSchema)
                    };
                    params.push(param);
                }
                return params;
     ...
     */
    extractResult(outputSchema, procedurePath) {
     ...
     */
    zodSchemaToOpenRPCSchema(zodSchema) {
        ...
        switch (def.typeName) {
            ...
            case 'ZodObject':
                ...
                if (def.shape) {
                    ...
                    for (const [key, fieldSchema] of Object.entries(shape)) {
                        properties[key] = this.zodSchemaToOpenRPCSchema(fieldSchema);
                    }
                }
            ...
            case 'ZodUnion':
                return { oneOf: def.options?.map((opt) => this.zodSchemaToOpenRPCSchema(opt)) || [] };
            case 'ZodOptional':
            case 'ZodDefault':

...
export class AIKeyValidator implements AIProviderValidator {
   ...
   */
  async validateKey(provider: string, apiKey: string): Promise<KeyValidationResult> {
   ...
   */
  private async validateAnthropicKey(apiKey: string): Promise<KeyValidationResult> {
   ...
   */
  private async validateGoogleKey(apiKey: string): Promise<KeyValidationResult> {
    try {
   ...
   */
  async validateMultipleKeys(keys: { provider: string; apiKey: string }[]): Promise<KeyValidationResult[]> {
    const validationPromises = keys.map(({ provider, apiKey }) =>
      this.validateKey(provider, apiKey)
    );

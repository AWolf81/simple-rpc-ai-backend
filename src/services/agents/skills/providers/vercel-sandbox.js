...
export class VercelSandboxProvider {
    ...
    async initialize(sandboxConfig) {
        try {
            ...
            this.SandboxClass = vercelSandbox.Sandbox;
            this.initialized = true;
            logger.debug('▲ Vercel Sandbox provider initialized');
        }
        catch (error) {
            throw new Error('@vercel/sandbox is not installed. Install it with: pnpm add @vercel/sandbox\n' +
     ...
     */
    async uploadScript(sandbox, request) {
     ...
     */
    async runScript(sandbox, request) {

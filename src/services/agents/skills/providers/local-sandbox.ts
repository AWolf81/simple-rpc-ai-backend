...
export class LocalSandboxProvider implements ISandboxProvider {
  ...
  private pythonGuardPath?: string;
  ...
  private resolveSandboxConfig(overrides?: Partial<SandboxConfig>): SandboxConfig {
  ...
  private validateScriptPath(scriptPath: string, sandbox: SandboxConfig): void {
    ...
    if (!isAllowed) {
      throw new Error(
        `Script path ${absolutePath} is outside allowed paths: ${sandbox.allowedPaths?.join(', ')}`
      );
    }
  }
   ...
   */
  private validatePath(pathToCheck: string, sandbox: SandboxConfig): void {
    ...
    if (!isAllowed) {
      throw new Error(
        `Path ${absolutePath} is outside allowed paths: ${sandbox.allowedPaths?.join(', ')}`
      );
    }
  }

  private applyRuntimeSecurity(params: {
    runtime: SkillScript['runtime'];
    command: string;
    args: string[];
    env: NodeJS.ProcessEnv;
    sandbox: SandboxConfig;
  }): { command: string; args: string[]; env: NodeJS.ProcessEnv; warnings: string[] } {
  ...
  private buildNodePermissionFlags(sandbox: SandboxConfig): { flags: string[]; warnings: string[] } {
    const warnings: string[] = [];
  ...
  private ensurePythonGuard(): string {
  ...
  private startViolationMonitor(): { stop: () => Promise<string[]> } | null {
  ...
  private prepareEnvironment(sandbox: SandboxConfig): NodeJS.ProcessEnv {
    ...
    if (!env.TMP && process.env.TMP) env.TMP = process.env.TMP;

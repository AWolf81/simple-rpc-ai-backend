...
export class SkillLoader {
  ...
  async loadAll(): Promise<Skill[]> {
    ...
    for (let i = 0; i < this.config.sources.length; i += maxConcurrent) {
      ...
      const batchSkills = await Promise.all(
        batch.map(source => this.load(source).catch(error => {
   ...
   */
  private async resolveSkillPath(source: SkillSource): Promise<string> {
    switch (source.type) {
      case 'builtin':
      ...
      default:
        throw new Error(`Unknown skill source type: ${(source as any).type}`);
    }
  }
   ...
   */
  private resolveBuiltinPath(source: BuiltinSkillSource): string {
   ...
   */
  private async downloadGitHubSkill(source: GitHubSkillSource): Promise<string> {
    // Parse GitHub URL
    ...
    try {
      ...
      await execAsync(
        `git clone --depth 1 --branch ${ref} https://github.com/${owner}/${repo}.git temp`,
        { cwd: targetDir }
      );
      ...
      const urlSource: UrlSkillSource = { type: 'url', url: zipUrl };
   ...
   */
  private async resolveNpmSkill(source: NpmSkillSource): Promise<string> {
   ...
   */
  private resolveLocalPath(source: LocalSkillSource): string {
   ...
   */
  private async downloadUrlSkill(source: UrlSkillSource): Promise<string> {
   ...
   * Extract ZIP skill
   */
  private async extractZipSkill(source: ZipSkillSource): Promise<string> {
   ...
   */
  private async extractZipFile(zipPath: string, targetDir: string): Promise<string> {
    await fs.mkdir(targetDir, { recursive: true });
   ...
   */
  private async loadFromPath(skillPath: string, source: SkillSource): Promise<Skill> {
   ...
   */
  async loadResources(skill: Skill): Promise<SkillResources> {
    if (skill.resources) {
   ...
   */
  private getCacheKey(source: SkillSource): string {
   ...
   */
  private estimateSkillSize(skill: Skill): number {
   ...
   */
  private async findSkillMd(dir: string): Promise<string | null> {

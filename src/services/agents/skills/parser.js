            ...
            metadata.scripts.forEach((script, idx) => {
                ...
                if (script.args !== undefined) {
                    if (!Array.isArray(script.args)) {
                        ...
                        script.args.forEach((arg, argIdx) => {
 ...
 */
export function validateSkillStructure(parsed, config) {
    const errors = [];
    const warnings = [];
    ...
    if (level1Tokens > config.maxLevel1Tokens) {
        warnings.push(`Metadata exceeds recommended limit: ${level1Tokens} tokens (max: ${config.maxLevel1Tokens})`);
    }
    ...
    if (level2Tokens > config.maxLevel2Tokens) {
        warnings.push(`Instructions exceed recommended limit: ${level2Tokens} tokens (max: ${config.maxLevel2Tokens})`);
        warnings.push('Consider moving detailed content to references/ directory');
    }
    // Metadata validation
 ...
 */
export async function loadSkillDirectory(basePath) {
    // Parse SKILL.md
 ...
 */
async function findFilesInDirectory(dirPath) {
    const files = [];
    try {
 ...
 */
export function extractSkillId(skillPath, metadata) {

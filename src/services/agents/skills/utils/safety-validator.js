...
export class SafetyValidator {
     ...
     */
    static CRITICAL_BLOCK_PATTERNS = [
     ...
     */
    static validate(scriptPath, args, scriptConfig, skillSafetyChecks) {
        const result = {
            allowed: true,
        ...
        if (scriptConfig?.dangerousArgs) {
            const hasDangerousArg = args.some(arg => scriptConfig.dangerousArgs.includes(arg));
            if (hasDangerousArg) {
                result.requiresApproval = true;
        ...
        if (scriptConfig?.maxTargets && args.length > scriptConfig.maxTargets) {
            result.requiresApproval = true;
            result.warnings.push(`⚠️  Operation affects ${args.length} targets (max: ${scriptConfig.maxTargets} without approval)`);
        }
     ...
     */
    static formatApprovalPrompt(scriptName, args, validation, cwd) {
        const lines = [
            '╔═══════════════════════════════════════════════════════════╗',
        ...
        lines.push(`🔒 Safety Level: ${validation.safetyLevel.toUpperCase()}`, '');
        ...
        lines.push('─────────────────────────────────────────────────────────────', '❓ Do you want to proceed with this operation?', '   Type "yes" to approve or "no" to cancel', '─────────────────────────────────────────────────────────────');
        return lines.join('\n');
    }
    /**
     ...
     */
    static validateNetworkOperation(method, url, requireApprovalForMutations = true) {
        const result = {
            allowed: true,
        ...
        if (mutationMethods.includes(normalizedMethod) && requireApprovalForMutations) {
            ...
            result.warnings.push(`⚠️  ${normalizedMethod} request to external server: ${url}`, `⚠️  This operation may modify data on the remote server`);
        }

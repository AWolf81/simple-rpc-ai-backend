...
export interface UserProfile {
  ...
  byokEnabled: boolean;
  byokProviders: Record<string, { enabled: boolean; apiKey?: string }>;
...
export class HybridUserService {
  ...
  async getUserProfile(userId: string): Promise<UserProfile | null> {
    const result = await this.db.query(
      `SELECT user_id, email, has_subscription, has_one_time_purchases, has_byok_configured,
  ...
  async getUserTokenBalances(userId: string): Promise<TokenBalance[]> {
    const result = await this.db.query(
      `SELECT id, user_id, balance_type, virtual_token_balance, total_tokens_purchased,
       ...
       ORDER BY consumption_priority ASC, created_at ASC`,
      [userId]
    );
   ...
   */
  async planConsumption(userId: string, tokensNeeded: number, apiKey?: string): Promise<ConsumptionPlan> {
    ...
    const plan: ConsumptionPlan['plan'] = [];
    const notifications: ConsumptionPlan['notifications'] = [];
    ...
    if (totalManagedTokens >= tokensNeeded) {
      ...
      for (const method of profile.consumptionOrder) {
        ...
        if (method === 'byok') continue; // Skip BYOK in managed-only strategy
  ...
  async executeConsumption(
    ...
    const actualConsumption: ConsumptionResult['actualConsumption'] = [];
    ...
    try {
          ...
          if (step.type === 'subscription' || step.type === 'one_time') {
            ...
            const result = await client.query(
              `UPDATE user_token_balances 
               ...
               RETURNING virtual_token_balance`,
              [step.tokensToConsume, step.tokensToConsume, step.balanceId]
            );
      ...
      await client.query(
        `INSERT INTO token_consumption_log 
  ...
  async addTokens(
    ...
    await this.db.query(
      ...
      [balanceId, userId, balanceType, usableTokens, tokens, platformFee, purchaseSource, expiryDate, priority]
    );

    // Update user profile capabilities
    const updateField = balanceType === 'subscription' ? 'has_subscription' : 'has_one_time_purchases';
    await this.db.query(
      `UPDATE user_profiles SET ${updateField} = TRUE, updated_at = CURRENT_TIMESTAMP WHERE user_id = $1`,
      [userId]
    );
  ...
  async configureBYOK(
    ...
    await this.db.query(
      `UPDATE user_profiles 
       ...
       WHERE user_id = $3`,
      [enabled, JSON.stringify(providers), userId]
    );
  }
   ...
   */
  async getConsumptionHistory(userId: string, limit: number = 50): Promise<any[]> {
    const result = await this.db.query(
      `SELECT id, request_id, total_tokens_needed, consumption_plan, actual_consumption,

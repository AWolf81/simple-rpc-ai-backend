...
export interface FreeTierModel {
  id: string;
  name: string;
  provider: string; // Any provider now supported
...
export interface FreeTierConfig {
  ...
  serverApiKeys?: Record<string, string>;
  defaultLimits?: FreeTierLimits;
}

export class FreeTierService {
  private usage: Map<string, FreeTierUsage> = new Map();
  private serverApiKeys: Map<string, string> = new Map();
  private freeModels: FreeTierModel[] = [];
  private enabled: boolean = false;

  constructor(config?: FreeTierConfig) {
  ...
  private initializeModels(config?: FreeTierConfig): void {
   ...
   */
  getAvailableModels(): FreeTierModel[] {
    if (!this.enabled) {
      return [];
    }
  ...
  ): Promise<{ allowed: boolean; reason?: string; limits?: FreeTierLimits }> {
    if (!this.enabled) {
   ...
   */
  async getUserUsage(userId: string, modelId: string): Promise<FreeTierUsage> {
   ...
   */
  configureServerKeys(keys: Record<string, string>): void {
   ...
   */
  addFreeModel(model: FreeTierModel): void {
    if (!this.enabled) {
   ...
   */
  private resetCountersIfNeeded(usage: FreeTierUsage, now: Date): void {

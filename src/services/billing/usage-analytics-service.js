...
export class UsageAnalyticsService {
    ...
    async getUserUsageHistory(userId, limit = 50) {
        const result = await this.db.query(`SELECT id, user_id, user_type, provider, model, input_tokens, output_tokens,
     ...
     */
    async getUserUsageSummary(userId, days = 30) {
       ...
       FROM usage_analytics 
       WHERE user_id = $1 AND timestamp >= $2`, [userId, cutoffDate]);
        const providerBreakdown = await this.db.query(`SELECT 
         provider,
         COUNT(*) as requests,
       ...
       GROUP BY provider
       ORDER BY tokens DESC`, [userId, cutoffDate]);
    ...
    async getUserStatus(userId) {
        ...
        const tokenAccount = await this.db.query('SELECT email, created_at FROM user_token_accounts WHERE user_id = $1', [userId]);
        // Check purchase history
        const purchases = await this.db.query(`SELECT COUNT(*) as purchase_count, 
    ...
    async getUserPurchaseHistory(userId, limit = 20) {
        const result = await this.db.query(`SELECT id, user_id, payment_id, purchase_type, variant_id, quantity,
    ...
    async isPaymentProcessed(paymentId) {
        const result = await this.db.query('SELECT COUNT(*) as count FROM user_purchases WHERE payment_id = $1', [paymentId]);

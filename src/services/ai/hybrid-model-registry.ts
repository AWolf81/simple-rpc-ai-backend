...
interface ProductionModelData {
  ...
  pricingOverride: { input: number; output: number; inputCacheHit?: number } | null;
  verified: string;
}
...
interface ProviderMapping {
  ...
  reason: string;
  models: Record<string, ProductionModelData>;
}
...
const productionModels = productionModelsData as ProductionModels;

export interface HybridModel {
  // Registry data (pricing, context, features)
  id: string;                    // Registry alias (claude-opus-4-1)
  productionId: string;          // Production versioned ID (claude-opus-4-1-20250805)
  name: string;
  provider: string;
  
  // Pricing (from registry or override)
  cost: {
    input: number;               // Per 1M tokens
    output: number;              // Per 1M tokens
    inputCacheHit?: number;      // For caching providers
  };
  
  // Capabilities (from registry)
  limit: {
    context: number;             // Context window in K tokens
    output?: number;             // Max output tokens in K
  };
  ...
  production: {
    ...
    requiresVersioning: boolean; // Whether this provider needs versioning
    deprecatedSince?: string;    // When the model was deprecated
    recommendedReplacement?: string; // Recommended replacement model
    deprecationReason?: string;  // Why the model was deprecated
  };
...
export class HybridModelRegistry {
  private cache: Map<string, HybridModel[]> = new Map();
   ...
   */
  supportsProductionMapping(provider: string): boolean {
   ...
   */
  async getModelsByProvider(provider: string): Promise<HybridModel[]> {
    ...
    const productionMapping = productionModels.providers[provider as keyof typeof productionModels.providers];
    ...
    const hybridModels: HybridModel[] = [];

    for (const registryModel of registryModels) {
      const modelId = registryModel.id;
      const productionModel = productionMapping.models[modelId as keyof typeof productionMapping.models];
   ...
   */
  async getProductionModel(provider: string, mode: 'best' | 'fast' | 'balanced' = 'best'): Promise<HybridModel> {
   ...
   */
  async getModelById(provider: string, modelId: string): Promise<HybridModel | null> {
    const models = await this.getModelsByProvider(provider);
   ...
   */
  async validateProductionMapping(): Promise<{
    valid: boolean;
    errors: string[];
    warnings: string[];
  }> {
    ...
    for (const [provider, mapping] of Object.entries(productionModels.providers)) {
      try {
        ...
        const missingInProduction = registryIds.filter(id => 
          !mapping.models[id as keyof typeof mapping.models]
        );
   ...
   */
  private shouldIncludeModel(model: HybridModel): boolean {

...
import { BitwardenCLI, BitwardenConfig } from './BitwardenCLI.js';
import * as winston from 'winston';
import { randomUUID } from 'crypto';

export interface UserIdentityMapping {
  opensaasUserId: string;     // "opensaas_user_12345" from JWT
  email: string;              // "user@gmail.com" from JWT
  vaultwardenUserId: string;  // UUID or email for Vaultwarden
...
export class UserIdentityBridge {
  ...
  private mappings: Map<string, UserIdentityMapping> = new Map();
  private bitwarden?: BitwardenCLI;
   ...
   */
  async getUserMapping(jwtPayload: OpenSaaSJWTPayload): Promise<UserIdentityMapping> {
   ...
   */
  private generateVaultwardenUserId(jwtPayload: OpenSaaSJWTPayload): string {
   ...
   */
  private async ensureVaultwardenUser(mapping: UserIdentityMapping): Promise<void> {
    try {
      if (!this.bitwarden) {
   ...
   */
  async getVaultwardenUserId(opensaasUserId: string): Promise<string | null> {
   ...
   */
  async getUserTier(opensaasUserId: string): Promise<string | null> {
    const mapping = this.mappings.get(opensaasUserId);
  ...
  validateJWT(token: string): OpenSaaSJWTPayload {
    ...
    try {
      ...
      return payload as OpenSaaSJWTPayload;
    } catch (error) {

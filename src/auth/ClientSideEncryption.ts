...
export interface EncryptionResult {
  ...
  keyDerivation: 'PBKDF2-SHA256'; // Fallback when Argon2id not available
}
...
export class ClientSideEncryption {
   ...
   */
  static deriveMasterKey(password: string, salt?: Uint8Array, iterations = this.ITERATIONS): {
    masterKey: Buffer;
    salt: Uint8Array;
  } {
  ...
  static validateEncryptionParams(encryptedData: DecryptionParams): boolean {
    ...
    for (const field of required) {
      if (!encryptedData[field as keyof DecryptionParams]) {
        return false;
      }
    }

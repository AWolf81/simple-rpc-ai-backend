...
export class UsageTracker {
   ...
   */
  calculateCost(provider: string, model: string, inputTokens: number, outputTokens: number): { cost: number; platformFee: number; totalCost: number } {
    const providerPricing = PROVIDER_PRICING[provider as keyof typeof PROVIDER_PRICING];
    ...
    if (!modelPricing) {
      ...
      const models = Object.values(providerPricing) as { input: number; output: number }[];
   ...
   */
  async recordUsage(event: Omit<UsageEvent, 'totalTokens' | 'cost' | 'platformFee' | 'totalCost'>): Promise<UsageEvent> {
    const totalTokens = event.inputTokens + event.outputTokens;
    const { cost, platformFee, totalCost } = this.calculateCost(
      event.provider,
      event.model || 'default',
      event.inputTokens,
      event.outputTokens
   ...
   */
  private async updateUserQuota(userId: string, tokens: number, cost: number): Promise<void> {
    // Get current quota or create if doesn't exist
    const existingQuota = await this.db.get(
    ...
    if (existingQuota) {
      ...
      await this.db.execute(
        ...
        [userId, 'starter', 10000, tokens, cost, resetDate.toISOString().split('T')[0]]
      );
    }
  }
   ...
   */
  async getUserUsage(userId: string, period: 'current_month' | 'last_30_days' | 'last_7_days' = 'current_month'): Promise<UsageSummary> {
    ...
    const topProviders = providerUsage.map((p: any) => ({
      provider: p.provider,
      usage: p.tokens,
    ...
    return {
      ...
      topProviders,
      dailyBreakdown: dailyUsage.map((d: any) => ({
        date: d.date,
        tokens: d.tokens || 0,
        cost: d.cost || 0,
   ...
   */
  async checkQuotaExceeded(userId: string): Promise<boolean> {
    const quota = await this.db.get(
      'SELECT tokens_used, monthly_token_limit FROM user_quotas WHERE user_id = ?',
      [userId]
    );
  ...
  async getUsageAnalytics(startDate?: Date, endDate?: Date): Promise<{
    ...
    providerBreakdown: Array<{ provider: string; tokens: number; cost: number; requests: number; percentage: number }>;
    tierBreakdown: Array<{ tier: string; users: number; tokens: number; cost: number; percentage: number }>;
  }> {
    let whereClause = '';
    let params: any[] = [];
    ...
    return {
      ...
      providerBreakdown: providerStats.map((p: any) => ({
        provider: p.provider,
        tokens: p.tokens || 0,
        cost: p.cost || 0,

...
export interface BillingConfig {
  ...
  quotaWarningThresholds: number[]; // e.g., [80, 95] for 80% and 95% warnings
}

export interface SubscriptionInfo {
  userId: string;
  tier: string; // Made flexible to support any tier
...
export class BillingEngine {
   ...
   */
  async createBillingEvent(event: Omit<BillingEvent, 'timestamp'>): Promise<string> {
   ...
   */
  async processUsageEvent(usageEvent: UsageEvent): Promise<void> {
   ...
   */
  async checkQuotaAndAlert(userId: string): Promise<void> {
      ...
      if (quotaStatus.quotaPercentage >= threshold) {
        ...
        const existingAlert = await this.db.get(
          `SELECT id FROM quota_alerts 
           WHERE user_id = ? AND threshold_percentage = ? AND billing_period = ?`,
          [userId, threshold, currentPeriod]
        );
        ...
        if (!existingAlert) {
          ...
          await this.db.execute(
            `INSERT INTO quota_alerts (id, user_id, threshold_percentage, billing_period)
             VALUES (?, ?, ?, ?)`,
            [
   ...
   */
  private async sendQuotaAlert(userId: string, threshold: number, quotaStatus: any): Promise<void> {
    try {
   ...
   */
  private async processBillingEvent(eventId: string): Promise<void> {
    try {
      const event = await this.db.get(
        'SELECT * FROM billing_events WHERE id = ? AND processed = FALSE',
        [eventId]
      );
      ...
      // Mark as processed
      await this.db.execute(
        'UPDATE billing_events SET processed = TRUE WHERE id = ?',
        [eventId]
      );
   ...
   */
  private async sendToOpenSaaS(event: any, metadata: any): Promise<void> {
    if (!this.config.opensaasWebhookUrl) {
   ...
   */
  private async sendToStripe(event: any, metadata: any): Promise<void> {
   ...
   */
  private async sendToLemonSqueezy(event: any, metadata: any): Promise<void> {
   ...
   */
  async handleQuotaExceeded(userId: string): Promise<{ action: 'block' | 'allow_overage' | 'upgrade_prompt'; message: string }> {
  ...
  async getSubscriptionInfo(userId: string): Promise<SubscriptionInfo | null> {
    const subscription = await this.db.get(
      'SELECT * FROM subscription_info WHERE user_id = ?',
      [userId]
    );
   ...
   */
  async updateSubscriptionInfo(userId: string, info: Partial<SubscriptionInfo>): Promise<void> {
    ...
    await this.db.execute(
      `UPDATE subscription_info SET ${updates.join(', ')} WHERE user_id = ?`,
      values
    );
   ...
   */
  async getBillingAnalytics(startDate?: Date, endDate?: Date): Promise<{
    totalRevenue: number;
    totalPlatformFees: number;
    totalEvents: number;
    eventsByType: Record<string, number>;
    revenueByTier: Record<string, number>;
    topUsers: Array<{ userId: string; revenue: number; events: number }>;
  }> {
    let whereClause = '';
    let params: any[] = [];
    ...
    return {
      ...
      revenueByTier: revenueByTier.reduce((acc: any, row: any) => {
   ...
   */
  private async processOpenSaaSWebhook(payload: any): Promise<void> {
   ...
   */
  private async processLemonSqueezyWebhook(payload: any): Promise<void> {

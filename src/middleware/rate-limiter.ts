 ...
 */
export const DEFAULT_TIER_LIMITS: Record<string, RateLimits> = {
  starter: {
 ...
 */
function tierConfigToRateLimits(tierConfig: SubscriptionTierConfig): RateLimits {
  return {
...
export class RateLimiter {
  ...
  private enableConcurrencyLimit: boolean;

  // In-memory fallback for when Redis is not available
  private memoryStore: Map<string, { count: number; resetTime: number; tokens: number }> = new Map();
  ...
  private async initializeRedis(): Promise<void> {
    try {
      ...
      console.warn('⚠️  Redis connection failed, using in-memory rate limiting:', (error as Error).message);
      this.redis = null;
    }
  }
  ...
  middleware = () => {
    return async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
      try {
        ...
        // Check RPM limit
        const rpmResult = await this.checkRateLimit(
          `rpm:${userId}`,
   ...
   */
  async checkTokenRateLimit(userId: string, tier: string, estimatedTokens: number): Promise<RateLimitResult> {
   ...
   */
  async recordTokenUsage(userId: string, tier: string, actualTokens: number): Promise<void> {
    const limits = this.getTierLimits(tier);
   ...
   */
  private getUserLimits(req: AuthenticatedRequest): RateLimits {
    const tier = req.authContext?.subscriptionTier || 'anonymous';
   ...
   */
  private async checkConcurrentRequests(userId: string, limit: number): Promise<RateLimitResult> {
   ...
   */
  private incrementConcurrentRequests(userId: string): void {
    const current = this.concurrentRequests.get(userId) || 0;
   ...
   */
  private decrementConcurrentRequests(userId: string): void {
    const current = this.concurrentRequests.get(userId) || 0;
    if (current <= 1) {
   ...
   */
  private sendRateLimitError(res: Response, message: string, result: RateLimitResult): void {
    res.status(429).json({
      error: {
        code: -32002,
        message,
   ...
   */
  async getRateLimitStatus(userId: string, tier: string): Promise<{
    rpm: RateLimitResult;
    tpm: RateLimitResult;

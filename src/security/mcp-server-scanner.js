 ...
 */
function scanFileContent(content, filename, lang) {
...
function resolvePackageName(config) {
...
async function fetchPyPIMetadata(packageName) {
    ...
    const sdist = releaseFiles.find((f) => f.packagetype === 'sdist' || f.filename?.endsWith('.tar.gz'));
    if (!sdist) {
 ...
 */
function scanDirectory(dir, lang) {
    const red = [];
    const yellow = [];
    let fileCount = 0;
 ...
 */
function classifySecurityLevel(metadata, redFlags, yellowFlags, isOfficial) {
    const reasons = [];
...
//# sourceMappingURL=mcp-server-scanner.js.map
...
export interface MCPRateLimitConfig {
  ...
  // Per-user authenticated limits (higher limits)
  authenticated: RateLimitConfig;
  
  // Per-tool specific limits
  toolLimits: Record<string, RateLimitConfig>;
  
  // Admin user limits (even higher)
  ...
  adaptive: {
    enabled: boolean;
    cpuThreshold: number;      // CPU usage % to start throttling
    memoryThreshold: number;   // Memory usage % to start throttling
    throttleMultiplier: number; // Multiply limits by this when throttling
  };
}

// Default rate limit configurations
export const DEFAULT_MCP_RATE_LIMITS: MCPRateLimitConfig = {
  enabled: true,
  global: {
...
export class MCPRateLimiter {
  ...
  private authenticatedLimiter: any;
   ...
   */
  public getMiddleware(toolName?: string): any[] {
    ...
    middlewares.push((req: AuthenticatedRequest, res: Response, next: NextFunction) => {
   ...
   */
  public createMCPToolMiddleware() {
   ...
   */
  public resetLimits(identifier: string) {
   ...
   */
  public updateConfig(newConfig: Partial<MCPRateLimitConfig>) {
 ...
 */
let defaultRateLimiter: MCPRateLimiter | null = null;

/**
 * Get or create default rate limiter instance
 */
export function getDefaultRateLimiter(config?: Partial<MCPRateLimitConfig>): MCPRateLimiter {
 ...
 */
export function createMCPRateLimit(config?: Partial<MCPRateLimitConfig>) {

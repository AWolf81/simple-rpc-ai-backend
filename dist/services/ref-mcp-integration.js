 ...
 */
import { MCPService, MCPUtils } from './mcp-service.js';
...
export class RefMCPIntegration {
    ...
    async initialize() {
        ...
        try {
            ...
            const refTools = tools.filter(tool => tool.name.startsWith('ref_') ||
     ...
     */
    async searchDocumentation(request) {
        if (!this.initialized) {
     ...
     */
    async searchCodeExamples(language, topic) {
     ...
     */
    async searchAPIDocumentation(apiName, method) {
     ...
     */
    async getGitHubDocumentation(owner, repo, path) {
     ...
     */
    async searchLocalDocs(query, projectPath) {
        if (projectPath) {
     ...
     */
    formatSearchResults(rawResults, scope) {
    ...
    determineResultType(result) {
        ...
        if (filename.includes('api') || content.includes('API'))
            return 'api';
        if (filename.includes('tutorial') || content.includes('tutorial'))
            return 'tutorial';
        if (filename.endsWith('.md') || filename.endsWith('.rst'))
            return 'markdown';
        if (filename.match(/\.(js|ts|py|java|go|rs|c|cpp)$/))
            return 'code';
     ...
     */
    determineResultSource(result, scope) {
        if (scope && scope !== 'all')
            return scope;
     ...
     */
    async findProjectDocumentation(projectPath) {
        const docFiles = [];
...
export class VSCodeRefIntegration {
     ...
     */
    static createForWorkspace(workspacePath, config = {}) {
     ...
     */
    static async searchVSCodeAPI(integration, query) {
     ...
     */
    static async getExtensionDocs(integration, extensionId) {

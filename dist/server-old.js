...
export function createAIServer(config) {
    // Complex server deprecated - use createSimpleAIServer instead
    throw new Error('createAIServer has been deprecated. Please use createSimpleAIServer from server-simple.js instead for a clean, working implementation.');
    ...
    // Vaultwarden auto-provisioning (improved auth flow)
    // Vaultwarden removed - use simple server instead
    // let vaultwardenProvisioning: VaultwardenAutoProvisioning | null = null;
    // let vaultwardenRPCMethods: VaultwardenRPCMethods | null = null;
    if (mode !== 'simple') {
        // Complex server mode deprecated - use createSimpleAIServer instead
        throw new Error('Complex server mode has been deprecated. Please use createSimpleAIServer from server-simple.js instead.');
    }
    ...
    if (config.oauthAuth && oauthAuthManager) {
        app.post('/auth/oauth', async (req, res) => {
            try {
                ...
                const { session, sessionToken } = await oauthAuthManager.authenticateWithOAuth(extensionId, provider, accessToken, deviceId);
        ...
        try {
            ...
            switch (method) {
                ...
                case 'admin.bulkGrantSpecialAccess':
                    ...
                    break;
                // Vaultwarden Auto-Provisioning Methods (Improved Auth Flow)
                case 'vaultwarden.onboardUser':
                    if (!vaultwardenRPCMethods)
                        throw new Error('Vaultwarden auto-provisioning not enabled');
                    result = await vaultwardenRPCMethods.onboardUser(params);
                    break;
                case 'vaultwarden.completeSetup':
                    if (!vaultwardenRPCMethods)
                        throw new Error('Vaultwarden auto-provisioning not enabled');
                    result = await vaultwardenRPCMethods.completeSetup(params);
                    break;
                case 'vaultwarden.getShortLivedToken':
                    if (!vaultwardenRPCMethods)
                        throw new Error('Vaultwarden auto-provisioning not enabled');
                    result = await vaultwardenRPCMethods.getShortLivedToken(params);
                    break;
                case 'vaultwarden.storeEncryptedKey':
                    if (!vaultwardenRPCMethods)
                        throw new Error('Vaultwarden auto-provisioning not enabled');
                    result = await vaultwardenRPCMethods.storeEncryptedKey(params);
                    break;
                case 'vaultwarden.retrieveEncryptedKey':
                    if (!vaultwardenRPCMethods)
                        throw new Error('Vaultwarden auto-provisioning not enabled');
                    result = await vaultwardenRPCMethods.retrieveEncryptedKey(params);
                    break;
                case 'vaultwarden.getAccountStatus':
                    if (!vaultwardenRPCMethods)
                        throw new Error('Vaultwarden auto-provisioning not enabled');
                    result = await vaultwardenRPCMethods.getAccountStatus(params);
                    break;
...
export async function createAIServerAsync(config) {
    // Complex server deprecated - use createSimpleAIServer instead
    throw new Error('createAIServerAsync has been deprecated. Please use createSimpleAIServer from server-simple.js instead for a clean, working implementation.');
    // Create AIService instance once here:
    ...
    return {
        ...
        start: (portOverride) => {
            ...
            const server = app.listen(serverPort, async () => {
                ...
                console.log(`🌐 Server running on port ${serverPort}`);
                console.log(`📋 Available custom functions: ${functionRegistry.listFunctions().map(f => f.name).join(', ')}`);
                // Vaultwarden organization validation removed - use createSimpleAIServer instead
            });
            return server;
        },
        stop: () => {
...
async function handleAIRequest(params, keyManager, aiService, config, authContext) {

...
export class BitwardenRESTAPI {
    ...
    async authenticateWithAPIKey() {
        ...
        try {
            ...
            const { stdout } = await execAsync('./node_modules/.bin/bw login --apikey --raw', { env: loginEnv });
    ...
    async unlockVault() {
        ...
        try {
            ...
            const { stdout: sessionToken, stderr } = await execAsync('./node_modules/.bin/bw unlock --raw --passwordenv BW_PASSWORD', { env });
     ...
     */
    async checkPersistentService() {
        try {
            this.logger.debug(`Quick check: Testing persistent service at ${this.apiBaseUrl}/status`);
            ...
            if (response.ok) {
                ...
                this.logger.debug(`Persistent service response: ${JSON.stringify(data)}`);
                return true;
            }
            return false;
        }
        catch (error) {
            this.logger.debug(`Persistent service quick check failed: ${error.message}`);
            return false;
        }
    }
    /**
    ...
    async startAPIServer() {
        ...
        return new Promise((resolve, reject) => {
            this.serverProcess = spawn('./node_modules/.bin/bw', ['serve', '--hostname', this.config.apiHost, '--port', this.config.apiPort.toString()], {
                env: {
                    ...process.env,
                    BW_SESSION: this.sessionToken
                },

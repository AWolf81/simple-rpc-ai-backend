...
(function (ValidatorType) {
    ValidatorType["ZOD"] = "zod";
    ValidatorType["YUP"] = "yup";
    ValidatorType["JOI"] = "joi";
...
export class SchemaRegistry {
    ...
    getJsonSchema(id) {
        ...
        switch (schema.validatorType) {
            case ValidatorType.ZOD:
                jsonSchema = this.convertZodToJsonSchema(schema.validator);
                break;
    ...
    convertZodToJsonSchema(zodSchema) {
        try {
            ...
            if (schema.$ref && schema.definitions) {
                ...
                const resolvedSchema = schema.definitions[refKey];
                if (resolvedSchema) {
    ...
    convertYupToJsonSchema(yupSchema) {
 ...
 */
export function createZodSchema(id, name, description, validator, options) {
    return {
        id,
        name,
        description,
 ...
 */
export function input(validator, id) {
 ...
 */
export function defineSchema(validator, options = {}) {
    ...
    if (validator._isYupSchema)
        validatorType = ValidatorType.YUP;
    if (validator.isJoi)
        validatorType = ValidatorType.JOI;
    const schemaDefinition = {
        id,
        name,
        description,

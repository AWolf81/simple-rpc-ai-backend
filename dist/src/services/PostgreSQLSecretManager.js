...
export class PostgreSQLSecretManager {
     ...
     */
    async storeUserKey(email, provider, apiKey) {
        try {
     ...
     */
    async getUserKey(email, provider) {
        try {
            const userId = this.getUserId(email);
            const secretKey = `${provider}_api_key`;
            const query = `
     ...
     * Delete user API key
     */
    async deleteUserKey(email, provider) {
        try {
            const userId = this.getUserId(email);
            const secretKey = `${provider}_api_key`;
            const query = `
     ...
     */
    async validateUserKey(email, provider) {
        try {
     ...
     */
    async logSecretAccess(email, operation, success, provider, errorMessage, ipAddress, userAgent) {
        try {
            const userId = this.getUserId(email);
            const query = `

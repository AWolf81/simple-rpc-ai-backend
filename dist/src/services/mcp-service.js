 ...
 */
import { MCPRegistryService, PREDEFINED_MCP_SERVERS } from './mcp-registry.js';
...
export class MCPService extends EventEmitter {
     ...
     */
    async registerPredefinedServers() {
     ...
     */
    async executeToolForAI(request) {
        if (!this.initialized) {
    ...
    findTools(pattern) {
        ...
        if (typeof pattern === 'string') {
            return allTools.filter(tool => tool.name.includes(pattern) ||
                tool.description.toLowerCase().includes(pattern.toLowerCase()));
        }
        else {
            return allTools.filter(tool => pattern.test(tool.name) ||
 ...
 */
let _defaultMCPService = null;
export function getDefaultMCPServiceInstance() {
    return _defaultMCPService;
}
export function setDefaultMCPServiceInstance(service) {
    _defaultMCPService = service;
}
/**
 * Create or get the default MCP service instance
 */
export function getDefaultMCPService(config) {
 ...
 */
export async function initializeDefaultMCPService(config) {

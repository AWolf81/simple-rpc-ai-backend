...
import { JWTMiddleware, mergeWithDefaultTiers } from '../auth/jwt-middleware.js';
...
export async function createMonetizedAIServer(config) {
    ...
    app.post('/webhooks/opensaas', express.raw({ type: 'application/json' }), async (req, res) => {
        try {
            const signature = req.headers['x-opensaas-signature'];
        ...
        try {
            ...
            switch (method) {
                case 'executeAIRequest':
                    result = await handleMonetizedAIRequest(params, req, aiService, usageTracker, billingEngine, rateLimiter, mergedTiers);
                    break;
                case 'getUsageStats':
    ...
    app.get('/admin/analytics', jwtMiddleware.authenticate, async (req, res) => {
        try {
 ...
 */
async function handleMonetizedAIRequest(params, req, aiService, usageTracker, billingEngine, rateLimiter, tiers) {
 ...
 */
async function handleGetUsageStats(params, req, usageTracker) {
 ...
 */
async function handleCheckQuotaStatus(req, usageTracker) {
    const userId = req.authContext?.userId;
    if (!userId) {
        throw new Error('Authentication required');
    }

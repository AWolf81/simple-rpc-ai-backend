...
export const promptProcedures = {
    ...
    getPrompts: publicProcedure
        ...
        if (mcpConfig?.extensions?.prompts?.customPrompts) {
            const customPrompts = mcpConfig.extensions.prompts.customPrompts.map((prompt) => ({
                name: prompt.name,
        ...
        if (input.search) {
            ...
            prompts = prompts.filter(p => p.name.toLowerCase().includes(searchLower) ||
...
function getPromptTemplate(name, ctx) {
...
function populateTemplate(messages, args) {
    return messages.map(message => ({
        ...message,
        content: {
            ...message.content,
            text: message.content.text.replace(/\{\{(\w+)\}\}/g, (match, key) => {

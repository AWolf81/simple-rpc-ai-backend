...
export function createAIRouter(config = {}, tokenTrackingEnabled = false, dbAdapter) {
    const mergedConfig = {
    ...
    return createTRPCRouter({
        ...
        getBYOKStatus: protectedProcedure
            ...
            return {
                ...
                hasConfiguredProviders: Object.values(providerStatus).some((p) => p.enabled && p.hasApiKey)
            };
        }),
        /**
        ...
        planConsumption: protectedProcedure
            ...
            const plan = await hybridUserService.planConsumption(ctx.user.userId, input.estimatedTokens, input.hasApiKey ? 'dummy-key' : undefined);
            return {

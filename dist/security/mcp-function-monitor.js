...
import { SecurityEventType, SecuritySeverity } from './security-logger.js';
import { zodToJsonSchema } from 'zod-to-json-schema';
export const DEFAULT_MCP_MONITOR_CONFIG = {
...
export class MCPFunctionMonitor {
    ...
    monitoringTimer;
    currentRouter;
    constructor(securityLogger, config = {}) {
        ...
        try {
            ...
            if (routerDef._def && routerDef._def.procedures) {
                for (const [name, procedure] of Object.entries(routerDef._def.procedures)) {
                    ...
                    const procedureWithMeta = procedure;
     ...
     */
    createSchemaSnapshot(toolName, procedure) {
        let jsonSchema = {};
        let metadata = {
        ...
        return {
            ...
            metadata: {
                ...
                description: procedure.meta?.mcp?.description,
                category: procedure.meta?.mcp?.category,
                version: procedure.meta?.mcp?.version
            }
        };
    }
    /**
     ...
     */
    analyzeSchemaMetadata(jsonSchema) {
        const metadata = {
            inputParameterCount: 0,
            requiredParameterCount: 0,
            hasFileInputs: false,
     ...
     */
    assessRemovalImpact(paramName, paramDef) {
     ...
     */
    assessAdditionImpact(paramName, paramDef) {
     ...
     */
    calculateSeverity(changes, changePercentage) {
     ...
     */
    calculateRiskScore(changes, metadata) {
        let score = 0;
        ...
        if (metadata.hasSystemAccess === true)
            score += 2;
        if (metadata.hasFileInputs === true)
            score += 1.5;
     ...
     */
    getRecommendations(severity, changePercentage, changes) {
     ...
     */
    async handleSchemaChange(analysis) {
     ...
     */
    isChangeAllowed(analysis) {
        return this.config.allowedChanges.some(allowed => allowed.toolName === analysis.toolName &&
     ...
     */
    changesMatchExpected(actualChanges, expectedChanges) {
        return actualChanges.every(change => expectedChanges.includes(`${change.type}:${change.field}`));
    }
    /**
     * Disable a tool due to security concerns
     */
    async disableTool(toolName, reason) {
     ...
     */
    async sendEscalationAlert(analysis) {
     ...
     */
    async checkForRemovedTools(currentProcedures) {
    ...
    async loadPersistedSchemas() {
        if (!this.config.persistSchemas || !this.config.schemaStoragePath)
            return;
        try {
     ...
     */
    approveSchemaChange(toolName, expectedChanges, validUntilDays = 7, approvedBy = 'admin') {
     ...
     */
    enableTool(toolName, approvedBy = 'admin') {
